...
class HexGrid(Grid):
    def __init__(self, width, height, co, ch):
        ...
        self.__positions = [[0]*height for i in range(width)]
        self.__trails = []
        self.__circle = -1
    ...
    def middleClick(self, x, y, img):
        if(self.__circle == -1):
            ...
            trail.floodFill(self.__matrix, None, None)
            ##trail.showPath(img)
            self.__co.subinscribeNext(MoveCommand(trail, self.__ch, self.__co, 0, self.__positions, self.__matrix))
