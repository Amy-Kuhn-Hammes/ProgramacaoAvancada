...
class Trail():
    ...
    def floodFill(self, matrix, pos, bl):
        ...
        if type(self.__type) != str:
            for i in range(len(self.__type)):
                for j in range(len(self.__type[0])):
                    if pos == None and self.__type[i][j].getCentralPoint().getX() == self.__start.getX() and self.__type[i][j].getCentralPoint().getY() == self.__start.getY():
                        temp = [i, j]       
        ...
        while len(fila) >0:
            ...
            for i in range(len(aux)):
                ...
                if x == endP[0] and y == endP[1]: 
                    ...
                    self.__path = temp
                    return temp 
                if x < len(matrix) and x >= 0 and y < len(matrix[0]) and y >= 0 and auxM[x][y] == 0 and matrix[x][y] == 0 and (pos == None or not (x == bl[0] and y == bl[1])):
                    auxM[x][y] = 1
                    temp = fila[0][1].copy()
                    temp.append([x,y])
    ...
    def getCentralPoint(self, pos):
        if type(self.__type) == str:
            return [self.__path[pos][0]*20+10,self.__path[pos][1]*20+10]
        else:
            return self.__type[self.__path[pos][0]][self.__path[pos][1]].getCentralPoint().getIntPoint()


    
    def showPath(self, img):
        for i in range(len(self.__path)-1):
            cv2.line(img, self.getCentralPoint(i), self.getCentralPoint(i+1) ,self.__color,2)
